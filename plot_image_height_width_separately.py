...
height_values = np.mean(image, axis=1)  # Average pixel values along height
width_values = np.mean(image, axis=0)   # Average pixel values along width

plt.figure(figsize=(10, 5))

# Plot for height direction
plt.subplot(1, 2, 1)
plt.plot(range(image.shape[0]), height_values, c='b')
plt.xlabel('Height')
...
plt.subplot(1, 2, 2)
plt.plot(range(image.shape[1]), width_values, c='b')
plt.xlabel('Width')
plt.ylabel('Pixel Value')
